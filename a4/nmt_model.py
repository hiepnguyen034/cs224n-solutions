...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        first_hidden = torch.zeros(2, b_size ,self.hidden_size, device=self.device).requires_grad_()
        first_cell = torch.zeros(2, b_size ,self.hidden_size,device=self.device).requires_grad_()

        enc_hiddens, (last_hidden,last_cell) = self.encoder(X,(first_hidden.detach(),first_cell.detach())
                             )

        enc_hiddens, seq_lens = nn.utils.rnn.pad_packed_sequence(sequence= enc_hiddens, batch_first=False)
        enc_hiddens = enc_hiddens.permute(1,0,2)

        concat_h = torch.cat((last_hidden[0],last_hidden[1]),dim=1) 
        concat_c = torch.cat((last_cell[0],last_cell[1]),dim=1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_steps = torch.split(Y, split_size_or_sections=1, dim=0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_unsqz = torch.unsqueeze(dec_hidden,dim=2)
        e_t_unsqz = torch.bmm(enc_hiddens_proj, dec_hidden_unsqz)
        e_t = torch.squeeze(e_t_unsqz,dim=2)
        ...
        alpha_t_unsqz = torch.unsqueeze(alpha_t,dim = 1)
        a_t = torch.bmm(alpha_t_unsqz,enc_hiddens)
